# ------------------------------------------------------------------------------
# ~~~
# Sets up the FUBUKI_CXX_WARNINGS variable according to the current compiler.
# #######################################
#
# output: sets the following variables:
# FUBUKI_GCC_WARNINGS: The warnings used when compiling with gcc.
# FUBUKI_MSVC_WARNINGS: The warnings used when compiling with MSVC.
# FUBUKI_CLANG_WARNINGS: The warnings used when compiling with clang.
# FUBUKI_CXX_WARNINGS: The warnings used for C++ files.
# ~~~
macro(fubuki_setup_warning_list)
    set(FUBUKI_GCC_WARNINGS
        -WNSObject-attribute
        -Waddress
    ...
    set(FUBUKI_MSVC_WARNINGS
        /Wall
        /wd4324 # Disabled - see below
        /wd4464 # Disabled - see below
...
# Set FUBUKI_CXX_WARNINGS after the first include()
...
# | -Wuseless-cast                          | Sometimes a type is not the same on Windows and on Linux, which makes a cast required for cross-platform builds.                              |
#
...
# | Wno-missing-designated-field-initializers | Useful for C. For C++, we can have default-constructed objects for non-specified values.         |
#
# ------------------------------------------------------------------------------
# MSVC
#
# | Warning | Reason to discard it                                                                                                    |
# |---------|-------------------------------------------------------------------------------------------------------------------------|
# | /wd4323 | This informs when using alignas() and when padding is actually added because of that. That's one use case for alignas.  |
# | /wd4464 | Basically required for MOC mechanisms (Qt, etc.) that generate code that use relative include paths.                    |
